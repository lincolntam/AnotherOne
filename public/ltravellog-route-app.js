...
async function initApp() {
    ...
    try {
        ...
        if (res.status === 401) {
            window.location.href = "/login?next=/ltravellog/trip-planner";
            return;
        }
        ...
        script.src = `https://maps.googleapis.com/maps/api/js?key=${config.Maps_API_KEY}&libraries=places&language=zh-HK&region=HK&callback=setupGoogleServices`;
        script.async = true;
        document.head.appendChild(script);
    } catch (error) {
        console.warn("Google Maps config unavailable; using fallback map.", error);
        setupFallbackMap();
    }
}
